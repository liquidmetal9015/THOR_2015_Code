 ...
 * 
 */
public class SuperVelocityPID implements  LiveWindowSendable{
	...
	public boolean onTarget() {
	    throw new RuntimeException("No tolerance value set when using SuperPID.onTarget()");
	}
    }

    private class PIDTask extends TimerTask {

        private SuperVelocityPID m_controller;

        public PIDTask(SuperVelocityPID controller) {
            if (controller == null) {
                throw new NullPointerException("Given SuperPID was null");
            }
     ...
     * @param output The  object that is set to the output percentage
     * @param period the loop time for doing calculations. This particularly effects calculations of the
     * integral and differential terms. The default is 50ms.
     */
    public SuperVelocityPID(double Kp, double Ki, double Kd, double Kf,
            PIDSource source, 
            double period) {
     ...
     * @param period 
     */
    public SuperVelocityPID(double Kp, double Ki, double Kd,
            PIDSource source, 
            double period) {
    ...
    public SuperVelocityPID(double Kp, double Ki, double Kd, double Kf,
            PIDSource source) {
        this(Kp, Ki, Kd, Kf, source, kDefaultPeriod);
    }
    ...
    /**
     * Set the setpoint for the SuperPID
    ...
    /**
     * Returns the current setpoint of the SuperPID
     ...
     * @deprecated Use setTolerance(Tolerance), i.e. setTolerance(new SuperPID.PercentageTolerance(15))
     */
     ...
     * creating the type of tolerance that you want to use: setTolerance(new SuperPID.AbsoluteTolerance(0.1))
    ...
    /**
     * Stop running the SuperPID, this sets the output to zero before stopping.
     */
    ...
    /**
     * Return true if SuperPID is enabled.
     */
