...
public class Robot extends IterativeRobot {
	...
	/*
	 * Things to do:
	 * Make code to zero the arms
	 * modify the 5 arm values
	 * look a the Lift heights for adjustment
	 * Adjust deadzones for the drive stick (I think It was drifting right)
	 * 
	 * 
	 * 
	 * Complications - 
	 * 	The arms do not zero at the same place and will need asymmetric values to work
	 * 		Add an offset or reprogram for seperate values for each arm.
	 */
	...
	// These values are top priority!!
	final double armOpen = 10;
	final double armClosed = 15;
	final double armScore = 10;
	final double armIn = 20;

	// This also needs to be changed.
	final double stepClose = 10;
	...
	// Minor adjustments will be needed
	final double rackHeight = 3.72;
	final double grabHeight = 1.3;
	// 1.453
	final double scoreHeight = 1.902;
	final double stepHeight = 2.65;
	...
	final boolean squaredInputs = false;
	final double max_speed = 2700;
	...
	public void autonomousInit() {/*
									 * controlL.disable(); controlR.disable();
									 * 
									 * autoloopR.setOutputRange(-0.5, 0.5);
									 * autoloopL.setOutputRange(-0.5, 0.5);
									 * autoloopR.enable(); autoloopL.enable();
									 * 
									 * 
	...
	public void autonomousPeriodic() {/*
										 * 
										 * SmartDashboard.putNumber("lifter2",
										 * lifter.get());
										 * SmartDashboard.putNumber("pot Value2"
										 * , lifterPot.getVoltage());
										 * SmartDashboard.putNumber("RightArm",
										 * armCodeR.get());
										 * SmartDashboard.putNumber("LeftArm",
										 * armCodeL.get());
										 * 
										 * 
										 * SmartDashboard .putNumber(
										 * "Right accuracy",
										 * ((controlL.getSetpoint() - nCodeL
										 * .getRate()) / nCodeL.getRate()) *
										 * 100);
										 * 
										 * SmartDashboard .putNumber(
										 * "Left accuracy",
										 ...
										 * SmartDashboard.putNumber(
										 * "LifterAccuracy2",
										 * ((lifterPID.getSetpoint() -
										 * lifterPot.getVoltage()) / lifterPot
										 * .getVoltage()) * 100);
										 */
		...
		if ((Math.abs(nCodeR.get()) < 2500 && Math.abs(nCodeL.get()) < 2500)) {
			motorR.set(-0.5);
			motorL.set(0.5);
		} else {
			motorR.set(0);
		...
		/*
		 * if(! armZeroR.get()){ armR.set(0.2); } else { armR.set(0);
		 * armCodeR.reset(); }
		 * 
		 * if(! armZeroL.get()){ armL.set(-0.2); } else { armL.set(0);
		 * armCodeL.reset(); }
		 */

		/*
		 * SmartDashboard.putNumber("nCode 234R", nCodeR.get());
		 * SmartDashboard.putNumber("ControlL setpoint 345",
		 * controlL.getSetpoint()); SmartDashboard.putNumber(
		 * "ControlR setpoint 567", controlR.getSetpoint());
		...
		/*
		 * if(done){ armControlL.setSetpoint(-123); time.delay(0.15);
	...
	public void teleopPeriodic() {
		/*
		 * if(stick.getRawButton(4)){ controlL.disable(); controlR.disable();
		 * 
		 * motorR.set(0.5); motorL.set(0.5); } else if (stick.getRawButton(5)) {
		 * control } else { controlL.enable(); controlR.enable();
		 * 
		 * }
		 */
		...
		/*
		 * if (auxStick.getRawButton(3)) { armsInLatch = true; }
		 * 
		 * if (armsInLatch && ! auxStick.getRawButton(3)) { armsInLatch = false;
		 * if(armsIn) armsIn = false; else armsIn = true; }
		 */
		...
		/*
		 * boolean liftSeqLatch = false; boolean liftSeqLift = false; boolean
		 * liftSeqTop = false; boolean liftSeqStall = false; boolean
		 * liftSeqStall2 = false; boolean liftSeqDrop = false;
		 */
		...
		// WHEN THE HEIGHT REACHES CLOSE TO THE TOP OR 3 SECONDS EXPIRES BEGIN A
		// TIMER TO DELAY THE CLAMP
		...
		// SET THE LIFTER SETPOINT BACK TO DEFAULT// TURNING LIFT SEQ DROP OFF
		// IS JUST A FORMALITY
		...
		if (liftSeqLift || liftSeqStall || score || auxStick.getRawButton(9) || auxStick.getRawButton(7)
				|| auxStick.getRawButton(11)) {
		...
		// IF A BUTTON ON THE PANEL IS PRESSED OR THE LIFT SEQUENCE IS IN THE
		// LIFTING, STALL1, OR STALL2
		...
		if (/* auxStick.getRawButton(7) || */liftSeqLift || liftSeqStall || liftSeqStall2) {
			...
			// 1.902
			lifting = false;
			// GRAB HEIGHT IS THE DEFAULT IF NONE OF THE ABOVE CONDITIONS ARE
			// MET
		} else {
			lifterPID.setSetpoint(grabHeight);
			// 1.504
		...
		// THIS IS FOR THE UNUSUAL CONDITION OF SUCKING TOTES OFF OF THE
		// SCORRING PLATFROM
		// IF THE INTAKE BUTTON IS DEPRESSED AND THE SCORE HEIGHT IS THE LIFTER
		// TARGET,
		// THE ARMS SHOULD CHANGE TO A NARROWER CONFIGURATION AND THE PULLER
		// WILL SUCK IN THE TOTE
		/*
		 * if(auxStick.getRawButton(11) && lifterPID.getSetpoint() ==
		 * scoreHeight){ pullerR.set(-1+((auxStick.getThrottle()+1)/2));
		 * pullerL.set(1-((auxStick.getThrottle()+1)/2)); sucking = true; score
		 * = true; //THIS IS THE DEFAULT CASE FOR TOTE INTAKE } else
		 */if (auxStick.getRawButton(10)) {
			pullerR.set(-1 + ((auxStick.getThrottle() + 1) / 2));
			pullerL.set(1 - ((auxStick.getThrottle() + 1) / 2));
			sucking = true;
			score = false;
			ejecting = false;
			// IF THE LIFTER IS AT EJECTION HEIGHT THE ARMS SHOULD NARROW TO
			// GRAB AND EJECT THE TOTE

			/*
			 * }else if (auxStick.getRawButton(10) && lifterPID.getSetpoint() ==
			 * scoreHeight) { pullerR.set((1-((auxStick.getThrottle()+1)/2)));
			 * pullerL.set(-1+((auxStick.getThrottle()+1)/2)); score = true;
			 * sucking = false; ejecting = true;
			 */
		...
		// IF THE LIFTER IS AT EJECTION HEIGHT THE ARMS SHOULD NARROW TO GRAB
		// AND EJECT THE TOTE
		...
		// THIS LOGIC INTERFACED THE JOYSTICK WITH THE WHEEL SPEEDS TO CREATE
		// ARCADE DRIVE
		if (moveValue > 0.0) {
			if (rotateValue > 0.0) {
		...
		// THESE ARE THE SPEED SCALING SETTINGS AND THE CODE THAT SETS THE WHEEL
		// SPEED
		...
		if (stick.getRawButton(1)) {
			...
			// I DON'T KNOW IF THESE BUTTONS ACTUALLY WORK BUT THEY SHOULD
			// SLOWELY ROTATE THE ROBOT
		...
		// THIS DEBUG BLOCK SHOWS THE PERCENT ERROR IN WHEEL VELOCITY AND LIFTER
		// POSITION.
		/*
		 * SmartDashboard .putNumber("Right accuracy", ((controlL.getSetpoint()
		 * - nCodeL .getRate()) / nCodeL.getRate()) * 100);
		 * 
		 * SmartDashboard .putNumber("Left accuracy", ((controlL.getSetpoint() -
		 * nCodeL .getRate()) / nCodeL.getRate()) * 100);
		 * 
		 * SmartDashboard.putNumber("LifterAccuracy", ((lifterPID.getSetpoint()
		 * - lifterPot.getVoltage()) / lifterPot .getVoltage()) * 100);
		 */
		...
		// I SHOULD CONSIDER THE IMPLICATIONS OF THE ORDER OF THIS CODE, MIGHT
		// BE NOTHING
		if (((moveValue < driveBackwardDead) && (moveValue > -driveForwardDead)) && (rotateValue < driveLeftDead)
		 ...
		 * Math.abs(nCodeR.getRate()); SmartDashboard.putNumber(
		 * "right top speed", topSpeedTestR); }
		 * 
		 * if (Math.abs(nCodeL.getRate()) > topSpeedTestL) { topSpeedTestL =
		 * Math.abs(nCodeL.getRate()); SmartDashboard.putNumber("left top speed"
		 * , topSpeedTestL); }
		 */
	}
	...
	public void testInit() {
		...
		lifterPID.disable();
		/*
		 * controlL.disable(); controlR.disable(); lifterPID.disable();
		 */
		 ...
		 * "The lifter appears to be going in the wrong direction", false);
		 * SmartDashboard .putBoolean("The Lifter is taking too long", false);
		 * 
		 * 
		 * Timer tooLong = new Timer();
		 * 
		 * boolean lifterIsFine = true; boolean wheelsAreFine = true; double
		 * close = 999; double closeLast = 999;
		 * 
		 * // Lifter diagnostics start here --------------------- if
		 * ((lifterPot.getVoltage() < 1.9) || (lifterPot.getVoltage() > 4.5)) {
		 * SmartDashboard.putBoolean("The Pot is not working", true);
		 * lifterIsFine = false; } lifterPID.enable();
		 * lifterPID.setSetpoint(2.5); while (!lifterPID.onTarget() &&
		 * lifterIsFine) { tooLong.start(); if (Math.abs(lifterPot.getVoltage()
		 * - 2.5) < close) { closeLast = close; close =
		 * Math.abs(lifterPot.getVoltage() - 2.5);
		 * 
		 * } time.delay(0.3); if (close > closeLast) {
		 * 
		 * lifterIsFine = false; lifterPID.disable(); lifter.set(0);
		 * SmartDashboard .putBoolean(
		 * "The lifter appears to be going in the wrong direction", true); } if
		 * (tooLong.get() > 3) { lifterIsFine = false; lifterPID.disable();
		 * lifter.set(0); SmartDashboard .putBoolean(
		 * "The Lifter is taking too long", true); }
		 * SmartDashboard.putNumber("LifterError", ((lifterPID.getSetpoint() -
		 * lifterPot.getVoltage()) / lifterPot .getVoltage()) * 100); }
		 * lifterPID.disable(); lifter.set(0); time.delay(2);
		 * 
		 * //Wheels test begins here if(wheelsAreFine) motorR.set(0.3);
		 * time.delay(1);
		 * 
		 * 
		 * 
